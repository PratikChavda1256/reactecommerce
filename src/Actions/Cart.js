...
import showToast from '../Utils/toast';
 ...
 export const addToCart =({_id, title, price, image,selectedQuentity},navigate) => async (dispatch) =>{
    try{      
        ...
        const response = await axios.post('http://localhost:3000/carts/', payload, config); // Adjust URL and endpoint as per your API setup
        dispatch({
 ...
 export const getCart = () => async (dispatch) =>{
    try{      
        ...
        const response = await axios.get('http://localhost:3000/carts/get-cart', config); // Adjust URL and endpoint as per your API setup
 ...
 export const removeFromCart = (productId) => async (dispatch) => {
    try {
        ...
        const response = await axios.post('http://localhost:3000/carts/remove-cart-item', { productId }, config);
        console.log(response.data);
...
export const updateCartQuantity = (productId, quantity) => async (dispatch) => {
    try {
        ...
        const response = await axios.post('http://localhost:3000/carts/update-cart-item-quantity', { productId, quantity }, config);
        console.log(response.data);
...
export const deleteCart = ()=> async(dispatch)=>{
    try{
        ...
        const response = await axios.delete('http://localhost:3000/carts/delete-cart', config);
